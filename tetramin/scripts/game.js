...
export class Game {
    ...
    placeTetromino() {
        ...
        for (let i = 0; i < positions.length; i++) {
            ...
            if (
                pos.row >= 0 && pos.row < this.boardTetris.rows &&
                pos.col >= 0 && pos.col < this.boardTetris.cols
            ) {
                this.boardTetris.matriz[pos.row][pos.col] = this.currentTetromino.id;
            } else {
    ...
    dropBlock() {
        if (!this.started || !this.currentTetromino) return;
        const drop = this.tetrominoDropDistance();
        this.currentTetromino.move(drop, 0);
        this.placeTetromino();
        this.currentTetromino.lastMoveWasRotation = false;
    }
    holdTetromino() {
    ...
    initializeHUD() {
        this.updateTextCanvas(this.canvasLinesCtx, 0);
        this.updateTextCanvas(this.canvasLevelCtx, 1);
        this.updateTextCanvas(this.canvasScoreCtx, 0);
    ...
    keyboard() {
        ...
        this.keyDownHandler = (evt) => {
            ...
            if (evt.key === "ArrowLeft") this.moveTetrominoLeft();
            if (evt.key === "ArrowRight") this.moveTetrominoRight();
            if (evt.key === "ArrowUp" && !this.keys.up) {
                this.rotateTetrominoCW();
                this.keys.up = true;
            }
            if (evt.key === "ArrowDown") this.keys.down = true;
            if (evt.key === " " && !this.keys.up && this.started && this.currentTetromino) {
                this.dropBlock();
                this.keys.up = true;
            }
            if (evt.key === "c" || evt.key === "C") this.holdTetromino();
        };
        ...
        this.keyUpHandler = (evt) => {
            ...
            if (evt.key === " ") this.keys.up = false;
        };

        // Primero limpiar si ya estaban
